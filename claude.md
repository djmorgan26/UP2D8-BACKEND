...
**Purpose**: Backend API service for UP2D8 application, handling live user requests, database interactions, and Gemini API calls
...
A FastAPI-based backend service for the UP2D8 application. It handles all live user requests from the frontend, manages MongoDB database interactions, and securely calls the Google Gemini API with search grounding.

### The Problem It Solves

UP2D8 backend provides:
- Secure API endpoints for user management, chat sessions, and feedback
- Azure Key Vault integration for secrets management (no secrets in code/env)
- Proxy to Gemini API with search grounding
- Session and message persistence in MongoDB
...
- **Auth**: Azure CLI (local), Managed Identity (production)

### Current Status

**Phase**: Active Development
**Features**: Chat API, User Management, Session Management, Feedback, Analytics
**Components**: API routes, dependencies, shared utilities
